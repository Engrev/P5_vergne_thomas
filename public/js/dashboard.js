$(document).ready(function () {
    ...
    $(document).on("click", ".post-published", function () {
        ...
        $.ajax({
            ...
            dataType: "json",
            method: "POST",
            data: {action: "activate", published: published, id_post: idPost},
            success: function (d) {
                switch (published) {
                    case "0":
                        parent.html("<i class='fas fa-check text-success post-published' data-state='1#"+idPost+"' data-toggle='tooltip' data-placement='bottom' title='Publier ?'></i>");
                        break;
                    case "1":
                        parent.html("<i class='fas fa-times text-danger post-published' data-state='0#"+idPost+"' data-toggle='tooltip' data-placement='bottom' title='Dépublier ?'></i>");
                        break;
                }
    ...
    $(document).on("click", ".comment-validated", function () {
        var obj = $(this);
        obj.tooltip("hide");
        var idComment = obj.data("id-comment");
        var parent = obj.parent();
        parent.html("<i class='fas fa-spinner fa-spin'></i>");
        ...
        $.confirm({
            ...
            buttons: {
                cancel: {
                    ...
                    btnClass: "btn-default",
                    action: function() {
                        parent.html("<i class='fas fa-check text-warning comment-validated' data-id-comment='"+idComment+"' data-toggle='tooltip' data-placement='bottom' title='Valider ?'></i>");
                    }
                },
                delete: {
                    text: "Supprimer",
                    ...
                    action: function() {
                        $.ajax({
                            ...
                            dataType: "json",
                            method: "POST",
                            data: {action: "delete", id_comment: idComment},
                            success: function (d) {
                ...
                confirm: {
                    ...
                    action: function() {
                        $.ajax({
                            ...
                            dataType: "json",
                            method: "POST",
                            data: {action: "validate", id_comment: idComment},
                            success: function (d) {
                                parent.parent().remove();
                            }
                        });
